<?php
/**
 * Toby Yates Theme functions.
 *
 * @package    WordPress
 * @subpackage Yates_Theme
 * @author     Greg Sweet <greg@ccdzine.com>
 * @copyright  Copyright (c) 2017 - 2018, Greg Sweet
 * @link       https://github.com/ControlledChaos/yates-theme
...
final class Functions {
	...
	public function __construct() {
		...
		// Page loader.
        add_action( 'yates_loader', [ $this, 'loader' ], 1 );
    ...
    public function loader() {
        ...
        if ( is_front_page() && is_home() ) {
            ...
            $loading = __( 'Loading Commercials', 'yates' );
        } elseif ( is_archive() ) {
            $loading = __( 'Loading Archives', 'yates' );
        } elseif ( is_search() ) {
            $loading = __( 'Loading Search Results', 'yates' );
        } elseif ( is_singular( 'features' ) ) {
            $loading = __( 'Loading Feature', 'yates' );
        } elseif ( is_singular( 'commercials' ) ) {
            $loading = __( 'Loading Commercial', 'yates' );
        } else {
            $loading = __( 'Loading', 'yates' );
        }
	...
	public function setup() {
		...
		if ( ! is_plugin_active( 'yates-plugin/yates-plugin.php' ) ) {
			add_image_size( __( 'meta-image', 'yates' ), 1200, 630, true );
		}
		 ...
		 */
		register_nav_menus( [
			'main'  => __( 'Main Menu', 'yates' ),
			'admin' => __( 'Admin Header Menu', 'yates' )
		] );
	...
	public function frontend_scripts() {
		...
		// HTML 5 support.
		wp_enqueue_script( 'yates-html5',  get_theme_file_uri( '/assets/js/html5.min.js' ), [], '' );
		wp_script_add_data( 'yates-html5', 'conditional', 'lt IE 9' );
	...
	public function frontend_styles() {
		...
		if ( $google ) {
			wp_enqueue_style( 'yates-fonts', 'https://fonts.googleapis.com/css?family=Open+Sans:400,400i,600,700,700i|Poppins:100,100i,200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i', [], null, 'screen' );
		}
 ...
 * @return object
 */
function yates_theme() {

	$yates_theme = Functions::get_instance();
